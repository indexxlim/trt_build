        ...
        if n.i().op == "Add":
            ...
            if found_pow:
                if version_check:
                    ...
                    n.attrs = {}
                    lower_bound = gs.Constant(
                        n.name + "/lower_bound", np.array(-64000.0, dtype=np.float32)
                    )
                    n.inputs = [n.inputs[0], lower_bound]

                    max_node_output = n.outputs[0]
                    # Max has already exist, avoid tensors with same names
                    max_node_output.name = max_node_output.name.replace(
                        "Cast", "ClipMax"
                    )

                    upper_bound = gs.Constant(
                        n.name + "/upper_bound", np.array(64000.0, dtype=np.float32)
                    )
                    min_node_inputs = [max_node_output, upper_bound]

                    min_node_output = gs.Variable(
                        max_node_output.name.replace("ClipMax", "ClipMin"),
                        dtype=np.float32,
                    )
                    min_node = gs.Node(
                        op="Min",
                        inputs=min_node_inputs,
                        outputs=[min_node_output],
                        attrs={},
                    )
                    ...
                    for o in max_node_output.outputs:
                        ...
                        if o.op != "Min":
                            o.inputs = [
                                min_node_output if i == max_node_output else i
                                for i in o.inputs
                            ]
                else:
...
def process_onnx(
    config: List[OnnxProcessOperation],
    onnx_input_fpath,
    onnx_output_fpath,
    keep_input=False,
    **kwargs,
