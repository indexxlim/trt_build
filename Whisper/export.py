...
class WhisperDecoderConverter(ModelFileConverter):
    ...
    def torch_to_onnx(
        ...
        if not network_metadata.other.kv_cache:
            ...
            def _export_forward(input_features, encoder_hidden_states, **kwargs):
                result = old_forward(
                    input_features, encoder_hidden_states, use_cache=False, **kwargs
                )
                return result[0]
            ...
            encoder_hidden_states = simplified_encoder(input_features).to(device)
            kv_decoder_input_ids = input_features[:, -1:].to(device)
            ...
            cross_attention_kv_generator = WhisperDecoderCrossAttentionKVGenerator(
