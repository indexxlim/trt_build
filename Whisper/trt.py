...
from Whisper.frameworks import WhisperFHuggingFace
from Whisper.WhisperModelConfig import WhisperModelTRTConfig, WhisperTRTBenchmarkingArgs
...
from Whisper.export import (
    WhisperDecoderONNXFile,
    WhisperEncoderONNXFile,
    WhisperDecoderTRTEngine,
...
class WhisperTRTEncoder(TRTHFRunner):
    ...
    def __init__(
        ...
        hf_config: PretrainedConfig,
        batch_size: int = 1,
        benchmarking_args: WhisperTRTBenchmarkingArgs = None,
    ):
        super().__init__(
    ...
    def forward(self, input_features, *args, **kwargs):
        ...
        is_cpu_mode = input_features.device == torch.device("cpu")
        ...
        # TODO: Could we just reuse input_features' data_ptr() as the first binding when input_features is already contiguous to
        # avoid an additional D2D?
        if is_cpu_mode:
            self.inputs["input_features"] = (
                input_features.int().flatten().contiguous().cuda()
            )
        ...
        self.trt_context.set_binding_shape(0, input_features.shape)
...
class WhisperTRTDecoder(TRTHFRunner):
    def __init__(
        self,
        trt_engine_file: TRTEngineFile,
        network_metadata: NetworkMetadata,
    ...
    def set_cross_attention_kv_cache_engine(self, cross_attention_kv_generator):
        ...
        with open(self.cross_attention_kv_generator.fpath, "rb") as f:
            ...
            self.cross_attention_kv_generator_trt_engine = (
                trt_runtime.deserialize_cuda_engine(f.read())
            )
            self.cross_attention_kv_generator_trt_context = (
                self.cross_attention_kv_generator_trt_engine.create_execution_context()
            )
        self.cross_attention_bindings = [
            None
        ] * self.cross_attention_kv_generator_trt_engine.num_bindings
        self.cross_attention_bindings[0] = self.encoder_hidden_states.data_ptr()
        # Cross attention cache as outputs
        for i in range(self.num_decoder_layers):
            self.cross_attention_bindings[2 * i + 1] = self.cross_attention_cache[
                f"past_key_values.{i}.encoder.key"
            ].data_ptr()
            self.cross_attention_bindings[2 * i + 2] = self.cross_attention_cache[
    ...
    def forward(
        ...
        if self.config.use_cache:
            ...
            for i in range(self.num_decoder_layers):
                ...
                self.trt_context.set_binding_shape(
                    self.kv_cache_binding_offset + 4 * i + 1, self_attention_kv_shape
                )
        ...
        logits = self.hidden_states.flatten()[:logits_length].view(
            bs, input_length, self.config.vocab_size
        )
        if is_cpu_mode:
            logits = logits.cpu()
        ...
        if self.config.use_cache:
            ...
            for i in range(self.num_decoder_layers):
                self_attention_k_output = self.self_attention_cache[
                    f"key_values.{i}.decoder.key"
                ]
                self_attention_v_output = self.self_attention_cache[
                    f"key_values.{i}.decoder.value"
                ]
                if is_cpu_mode:
                ...
                present_key_values += (
                    (self_attention_k_output, self_attention_v_output),
                )
        ...
        return Seq2SeqLMOutput(
            logits=logits.to(self.return_device),
    ...
    def reset(self):
        """
        You should always call this function after a use case because WhisperTRTDecoder does not clear the cached encoder_hidden_states or cross_attention itself.
        """
        self.persist_encoder_hidden_states = False
...
class WhisperTRT(TRTInferenceCommand):
    def __init__(self):
        super().__init__(
            WhisperModelTRTConfig,
            "Runs trt results for Whisper model.",
            WhisperFHuggingFace,
        )
        self.whisper_trt_decoder = None
        self.whisper_trt_encoder = None
    ...
    def cleanup(
        ...
        # Deactivates context
        if self.whisper_trt_encoder:
            self.whisper_trt_encoder.release()
        if self.whisper_trt_decoder:
            self.whisper_trt_decoder.release()

        if not keep_trt_engine:
            self.whisper_trt_encoder_engine.cleanup()
            self.whisper_trt_decoder_engine.cleanup()
            # TODO: Avoid using workspace.metadata to handle additional removals.
            if workspace.metadata.other.kv_cache:
                self.whisper_trt_cross_attention_kv_generator.cleanup()
    ...
    def generate(
        ...
        decoder_output = self.whisper_trt_decoder.generate(
            input_features,
            max_length=max_length,
            min_length=min_length,
            num_beams=num_beams,
            early_stopping=early_stopping,
            eos_token_id=self.whisper_trt_decoder.config.eos_token_id,
            pad_token_id=self.whisper_trt_decoder.config.pad_token_id,
            use_cache=use_cache,
        ...
        self.whisper_trt_decoder.reset()
    ...
    def execute_inference(
        ...
        if not benchmarking_mode:
            output_seq_len = WhisperModelTRTConfig.MAX_OUTPUT_LENGTH[metadata.variant]
            input_ids = tokenizer(
        ...
        encoder_last_hidden_state, encoder_e2e_time = encoder_inference(
            self.whisper_trt_encoder, input_ids, timing_profile
        )
        ...
        models = NetworkModels(
            ...
            trt=[
                NetworkModel(
                    name=WhisperModelTRTConfig.NETWORK_DECODER_SEGMENT_NAME,
                    fpath=self.whisper_trt_decoder_engine.fpath,
                ),
                NetworkModel(
                    name=WhisperModelTRTConfig.NETWORK_ENCODER_SEGMENT_NAME,
                    fpath=self.whisper_trt_encoder_engine.fpath,
                ),
            ],
        )
    ...
    def _setup_engines(
        ...
        assert len(hash_onnx_fpath) == len(
            WhisperModelTRTConfig.NETWORK_SEGMENTS
        ), "There should only be {} exported ONNX segments in Whisper model.".format(
            len(WhisperModelTRTConfig.NETWORK_SEGMENTS)
        )

        decoder_onnx_fpath = hash_onnx_fpath[
            WhisperModelTRTConfig.NETWORK_DECODER_SEGMENT_NAME
        ].fpath
        encoder_onnx_fpath = hash_onnx_fpath[
            WhisperModelTRTConfig.NETWORK_ENCODER_SEGMENT_NAME
        ].fpath

        # Use HuggingFace WhisperConfig to set up parameter instead of harc-coded values.
        ...
        if hf_config.use_cache:
            ...
            for i in range(num_decoder_layers):
                dec_profiles = (
                    ...
                    .add(f"past_key_values.{i}.decoder.value", **self_attention_profile)
                    .add(f"past_key_values.{i}.encoder.key", **cross_attention_profile)
                    .add(
        ...
        self.whisper_trt_encoder_engine = WhisperEncoderONNXFile(
            encoder_onnx_fpath, metadata
        ).as_trt_engine(
            os.path.splitext(encoder_onnx_fpath)[0]
        ...
        self.whisper_trt_decoder_engine = WhisperDecoderONNXFile(
        ...
        # Create WhisperTRTEncoder and WhisperTRTDecoder instances.
        self.whisper_trt_encoder = WhisperTRTEncoder(
            self.whisper_trt_encoder_engine,
            metadata,
            hf_config,
            batch_size=batch_size,
            benchmarking_args=benchmarking_args,
        )
        self.whisper_trt_decoder = WhisperTRTDecoder(
            self.whisper_trt_decoder_engine,
            metadata,
            hf_config,
            batch_size=batch_size,
            num_beams=num_beams,
        ...
        if metadata.other.kv_cache:
            ...
            cross_attention_kv_generation_profiles = [
                Profile().add(
                    "encoder_hidden_states",
                    min=(batch_size * num_beams, 1, encoder_hidden_size),
                    opt=(
            ...
            decoder_onnx_fpath_kv_generator = os.path.join(
                decoder_folder,
                "cross_attention_kv_generator",
                decoder_name + "-cross_attention_kv_generator" + decoder_ext,
            )
            self.whisper_trt_cross_attention_kv_generator = WhisperDecoderONNXFile(
                decoder_onnx_fpath_kv_generator, metadata
            ).as_trt_engine(
                os.path.splitext(decoder_onnx_fpath_kv_generator)[0]
                + "-{}.engine".format(engine_tag),
                profiles=cross_attention_kv_generation_profiles,
                preview_features=preview_features,
            )

            self.whisper_trt_decoder.set_cross_attention_kv_cache_engine(
                self.whisper_trt_cross_attention_kv_generator
            )
        ...
        try:
            if not benchmarking_mode:
                self._setup_engines(
                    ...
                    batch_size,
                    args.num_beams,
                    disable_preview_dynamic_shapes,
                )
                ...
                max_input_seq_len = WhisperModelTRTConfig.MAX_SEQUENCE_LENGTH[
                    metadata.variant
                ]
                max_output_seq_len = WhisperModelTRTConfig.MAX_OUTPUT_LENGTH[
                    metadata.variant
                ]
                ...
                benchmarking_args = WhisperTRTBenchmarkingArgs(
                    input_seq_len,
                    output_seq_len,
                    input_profile_max_len,
                    output_profile_max_len,
